...
function openModal(data) {
    ...
    // Inject tools badges
    if (mTools && data.tools) {
        const tools = data.tools.split(',');
        mTools.innerHTML = tools.map(tool => 
            `<span class="px-3 py-1 bg-slate-100 dark:bg-white/10 rounded-full text-xs font-medium text-slate-600 dark:text-slate-300">${tool.trim()}</span>`
        ).join('');
    }
...
contactForm.addEventListener('submit', (e) => {
    ...
    // Kirim data via AJAX ke Formsubmit.co
    ...
    .then(response => {
        if (response.ok) {
            showToast('Pesan berhasil dikirim! Saya akan segera menghubungi Anda. ✅');
            contactForm.reset();
        } else {
            showToast('Gagal mengirim pesan. Coba lagi ya! ❌', 'error');
        }
    })
    .catch(() => {
        showToast('Gagal mengirim pesan. Coba lagi ya! ❌', 'error');
...
// === ORBIT ANIMATION SAFETY ===
// Pastikan orbit selalu berputar setelah page load
// Ini mencegah bug jika animasi CSS gagal ke-trigger
window.addEventListener('load', () => {
    ...
    // Pastikan hero-image-container visible setelah animasi masuk selesai
    setTimeout(() => {
        ...
        const heroProfile = document.querySelector('.hero-profile-img');
        if (heroProfile) {
            heroProfile.style.opacity = '1';
            heroProfile.style.filter = 'none';
        }
        const circles = document.querySelectorAll('.hero-circle-outer, .hero-circle-middle');
        circles.forEach(c => {
            c.style.opacity = '1';
        });
    }, 2000);
